...
`Glob "knowledge/**/*.md"` (covers default + any custom KBs). Read filenames. The triage agent already searched but might have missed something. Try once more with the admin's perspective on the question. If you find a match, point the admin at it: *"This article looks relevant — should I just send the user the answer from `knowledge/how-to-reset-vpn.md`?"*
...
- Confirm with the admin before writing. On approval, `Write` to `knowledge/<filename>.md` (unless the admin asked for a different KB collection — then `knowledge/<that>/<filename>.md`).
...
Captured the answer as `knowledge/how-to-reset-vpn-password.md`.
```
